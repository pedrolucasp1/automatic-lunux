...
case $option in
    1)
        ...
        echo " " >> /etc/apt/sources.list
        echo " " >> /etc/apt/sources.list
        echo "# deb cdrom:[Debian GNU/Linux 10.12.0 _Buster_ - Unofficial amd64 NETINST with firmware 20220326-11:22]/ buster contrib main non-free" >> /etc/apt/sources.list
        echo " " >> /etc/apt/sources.list
        echo "#deb cdrom:[Debian GNU/Linux 10.12.0 _Buster_ - Unofficial amd64 NETINST with firmware 20220326-11:22]/ buster contrib main non-free" >> /etc/apt/sources.list
        echo " " >> /etc/apt/sources.list
        ...
        ufw allow 54222
        iptables -A INPUT -p tcp --dport 22 -j DROP & echo "y" | ufw enable
        clear
        read;;
